...
def _rows(transactions: List[Transaction]) -> List[dict]:
    """Normalize ORM objects to plain dicts — call while session is open."""
    result = []
    for t in transactions:
        account_name  = t.account.name  if t.account  else str(t.account_id)
        category_name = t.category.name if t.category else str(t.category_id)
        result.append({
            "id":          str(t.id),
            "date":        str(t.date),
            "type":        t.type.value,
...
class ExportService:
    ...
    def to_pdf(self, transactions: List[Transaction]) -> bytes:
        ...
        col_widths  = [70, 60, 80, 110, 110, 170]
